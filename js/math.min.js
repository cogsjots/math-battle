...
(function(R, r) {
		...
		switch (a) {
			...
			case "\u2013":
				if ("+" == a) {
					...
					(f = l(hard ? 10 : 0, hard ? 200 : 100)),
						(d = l(hard ? 10 : 0, hard ? 200 : 100)),
						(c = f + d);
				if (!b) {
					var e = Math.min(c, d);
					e = Math.min(e, f);
			...
			case "/":
				"\u00d7" == a
					? ((c = l(hard ? 3 : 1, hard ? 13 : 12)),
						(d = l(hard ? 3 : 1, hard ? 13 : 12)),
						(f = c * d))
					: ((f = l(hard ? 3 : 1, hard ? 13 : 12)),
						(d = l(hard ? 3 : 1, hard ? 13 : 12)),
						(c = f * d)),
					b ||
						((e = Math.min(c, d)),
	...
	function H() {
		var a = +u || "0";
		ha.innerHTML = a;
		ia.innerHTML = a + " out of " + totalProblems;
	}

	function I() {
		m && (ja.innerHTML = +m.x || "0", ka.innerHTML = m.op || "", la.innerHTML = +m.y || "0", ma.innerHTML = +m.res || "0")
	}
	...
	function G(a, b) {
		var c = (v - +new Date) / 3E4;
		0 > c && (c = 0);
		1 < c && (c = 1);
		a && (clearTimeout(Z), t(J, "animated"), Z = setTimeout(function() {
			w(J, "animated")
		}, 150));
	...
	function da(a, b, c) {
		...
		d.onreadystatechange = function() {
			4 == d.readyState && 200 == d.status && c(JSON.parse(d.responseText))
		};
		d.open("POST", a, !0);
		d.send(f.join("&"))
	}
	...
	function D(a) {
		x &&
			(!a === !m.correct
				? ((v += 1500),
					3E4 < v - +new Date() && (v = +new Date() + 3E4),
					u++,
					H())
				: ((v -= 4e3), na(),
					wrongAnswers.push({
						problem: m.x + " " + m.op + " " + m.y + " = " + m.res,
						promptedAnswer: m.correct ? "Correct" : "Wrong"
					})), // Store wrong answer
			V(!1, !a !== !m.correct),
			(m = T()),
			H(), // Update the score display
			I())
	}
	...
	function displayWrongAnswers() {
		var wrongAnswersContainer = g("wrong_answers");
	...
	var v, F, A = !1,
		...
		m, h = !1,
		u = 0,
		totalProblems = 0, // Initialize total problems answered
		wrongAnswers = [], // Initialize wrong answers array
	...
	var k = {
		...
		check: function(a) {
			if (!a) return !1;
			do
				if (y(a, "button") || y(a, "score_share")) return a; while (a = a.parentNode);
			return !1
		}
	};
